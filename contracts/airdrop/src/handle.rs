...
pub fn try_claim(
    ...
    validation::validate_claim( &deps,
        info.clone(),
        eth_pubkey.clone(),
        eth_sig,
        config.clone(),
    ...
    // let redeem_amount = amount.clone();

    // If we want to have 100% allocation claim in 1tx, redeem_amount = amount.clone();
    total_claimed_w(deps.storage)
        .update(|claimed| -> StdResult<Uint128> { Ok(claimed + redeem_amount) })?;
...
pub fn claim_tokens(
    ...
    let mut redeem_amount = amount.to;
